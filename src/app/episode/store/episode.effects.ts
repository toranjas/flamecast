...
export class EpisodeEffects {
  ...
  createEpisode$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        from(
          this.storageProvider.createEpisode(
            action.episodeLocation,
            action.episodeInfo,
          ),
        ).pipe(
          ...
          ), // We call "Load" here because we are loading the newly-created episode
          catchError((e) => {
    ...
    this.store.select(selectEpisodeState).pipe(
      ...
      filter((episode) => {
        if (!this.hasLoadedOrCreated) {
          console.log(
            'Saving prevented. Episode has not loaded or been created yet.',
          );
          return false;
        }
        if (this.stopSavingChanges) {
          console.log(
            'Saving prevented. Episode is currently creating or loading.',
          );
          return false;
        }
        return true;

...
export class EpisodeEffects {
  ...
  createEpisode$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        from(this.storageProvider.createEpisode(action.episodeLocation, action.episodeInfo)).pipe(
  ...
  saveEpisode$ = createEffect(() =>
    this.store.select(selectEpisodeState).pipe(
      ...
      filter((after) => after !== undefined) as OperatorFunction<Episode | undefined, Episode>,

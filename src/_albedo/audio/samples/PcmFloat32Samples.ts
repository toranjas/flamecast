...
export class PcmFloat32Samples {
  /**
   ...
   */
  static getPeak = (samples: Float32Array): number => {
    let peak = 0.0;
   ...
   */
  static toDBFS = (sample: number): number => {
   ...
   */
  static samplesToPcmInt16 = (samples: Float32Array): Int16Array => {
    const result = new Int16Array(samples.length);
    for (let i = 0; i < samples.length; i++) {
      result[i] = PcmFloat32Samples.sampleToPcmInt16(samples[i]);
    }
    return result;
  };
   ...
   */
  static sampleToPcmInt16 = (sample: number): number => {
    ...
    return sample < 0 ? sample * INT16.minValue : sample * INT16.maxValue;
  };
}
